...
def readSeedPattern(seed_pattern_dir):
    seed_files = os.listdir(seed_pattern_dir)
    seed_pattern_data = {}
    for f in seed_files :
        pattern_name = f.split(".")[0]
        with open(os.path.join(seed_pattern_dir, f)) as data_file:
            seed_pattern_data[pattern_name] = json.load(data_file)
    return seed_pattern_data


def generateData(data_dir_save_to, seed_pattern_dict, number_per_pattern=5) :
    for patt_name in seed_pattern_dict.keys() :
        pattern = seed_pattern_dict[patt_name]
        pattern.extend(pattern)
        for i in range(1,number_per_pattern):
            waferid = "{0}_{1}".format(patt_name,i)
            waferMap = WaferMap(waferid)
            waferMap.initMap(100,100,pattern)
            waferMap.setTitle(patt_name)
            waferMap.save(data_dir_save_to + "/{0}".format(waferid))

def generateData_notused(data_dir) :
